...
class ScalaJSON(o: java.lang.Object) extends Seq[ScalaJSON] with Dynamic {
	...
	override def equals(obj: Any) = o.equals(obj)
	...
	def toMap: Map[String, ScalaJSON] = o match {
		case a: JSONObject => a.entrySet.map { entry => entry.getKey -> new ScalaJSON(entry.getValue) } toMap
		case _ => throw new JSONException
	}
